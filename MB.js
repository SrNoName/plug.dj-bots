...
function callback(data){
    if (data.message.indexOf("-stop") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        ...
        if (data.message.indexOf("808, go into sleep mode") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        script = false,
        API.on(API.DJ_ADVANCE, newdj)
        status = "At idle";
    ...
    if (data.message.indexOf("-start") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
       ...
       if (data.message.indexOf("808, start up") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        script = true;
        API.on(API.DJ_ADVANCE, newdj)
        status = "Running";
        console.log("[#808] started");
...
function command(data) {
    if (script && data.message.indexOf("-debug") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        ...
        if (script && data.message.indexOf("808, toggle debug mode") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        debug = !debug;
        if(announce)API.sendChat("@"+data.from+" Debug: "+debug);
        if(debug){updateChat("[#808] ","Debug mode toggled")}
    }
       ...
       if (script && data.message.indexOf("-announce") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        ...
        if(debug){updateChat("[#808] ","Announce mode toggled")}
           ...
           if (script && data.message.indexOf("808, toggle announce mode") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        announce = !announce;
        if(announce)API.sendChat("@"+data.from+" I will now announce events!");
        if(debug){updateChat("[#808] ","Announce mode toggled")}
    }
       if (script && data.message.indexOf("-clear") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        ...
        if(announce)API.sendChat("@"+data.from+" Chat cleared!");
           ...
           if (script && data.message.indexOf("808, clear chat") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        API.sendChat("/clear");
        if(announce)API.sendChat("@"+data.from+" Chat cleared!");
    }
    ...
    if (script && data.message.indexOf("-autowoot on") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        ...
        if (script && data.message.indexOf("808, turn autowoot on") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        wootmode = "Auto";
        if(mirror){
            mirror = false;
            if(debug){updateChat("[#808] ","Mirror turned off due to autowoot")}
    ...
    if (script && data.message.indexOf("-woot off") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        ...
        if(mirror){
            ...
            if (script && data.message.indexOf("808, turn autowoot off") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
        wootmode = "Off";
        if(mirror){
            mirror = false;
            if(debug) updateChat("[#808} ","Mirror turned of due to autowoot");
        ...
        if (script && data.message.indexOf("-mirror on") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
            ...
            if(announce)API.sendChat("@"+data.from+" I am now mirroring your votes!");
                ...
                if (script && data.message.indexOf("808, mirror my votes") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
            wootmode = "Mirror";
            if(autowoot){
                if(debug) updateChat("[#808} ","Autowoot turned off due to mirror");
                autowoot = false;
        ...
        if (script && data.message.indexOf("-mirror off") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
            ...
            if(announce)API.sendChat("@"+data.from+" I will not mirror your votes anymore.");
                ...
                if (script && data.message.indexOf("808, stop mirroring my votes") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
            wootmode = "Off";
            if(autowoot){
                autowoot = false;
                if(debug) updateChat("[#808]"," Autowoot turned off due to mirror");
        ...
        if (script && /-nick (.*)$/.exec(data.message) && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
         ...
         if (script && /808, change your name to (.*)$/.exec(data.message) && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getuser().id)) {
            Models.user.changeDisplayName(RegExp.$1);
            updateChat("[#808] ","Username changed by ,DerpTheBass'");
        }
        if (data.message.indexOf("-info") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
            ...
            if(debug){console.log("[#808] Sending status/info")}
               ...
               if (data.message.indexOf("808, give me your stats") > -1 && (data.fromID === "5105e7a23e083e5100cc1d96" || API.getUser().id)) {
            var elapsed = new Date().getTime() - joined;
            API.sendChat("@,DerpTheBass' I've been running for "+Math.round(elapsed/100000)+" minutes."+" Running on "+BrowserDetect.browser+" Version "+BrowserDetect.version+" on "+BrowserDetect.OS+". Woot mode: "+wootmode+ ". Debug: "+debug+" Status: "+status+" Announce: "+announce);
            if(debug){console.log("[#808] Sending status/info")}
        }
